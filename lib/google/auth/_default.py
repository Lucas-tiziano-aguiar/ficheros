...
from lib.google.auth import environment_vars
from lib.google.auth import exceptions
import lib.google.auth.transport._http_client
...
def _warn_about_problematic_credentials(credentials):
    ...
    """
    from lib.google.auth import _cloud_sdk
...
def _load_credentials_from_info(
    ...
    from lib.google.auth.credentials import CredentialsWithQuotaProject
...
def _get_gae_credentials():
    ...
    try:
        ...
        import lib.google.auth.app_engine as app_engine
    except ImportError:
...
def _get_gce_credentials(request=None, quota_project_id=None):
    ...
    # some cases where it's not available, so we tolerate ImportError.
    try:
        from lib.google.auth import compute_engine
        from lib.google.auth.compute_engine import _metadata
    except ImportError:
        _LOGGER.warning("Import of Compute Engine auth library failed.")
        return None, None

    if request is None:
        request = lib.google.auth.transport._http_client.Request()
...
def _get_external_account_credentials(
    ...
    if info.get("subject_token_type") == _AWS_SUBJECT_TOKEN_TYPE:
        ...
        from lib.google.auth import aws
        ...
        from lib.google.auth import pluggable
        ...
        try:
            ...
            from lib.google.auth import identity_pool
    ...
    if request is None:
        ...
        request = lib.google.auth.transport.requests.Request()
...
def _get_external_account_authorized_user_credentials(
    ...
    try:
        from lib.google.auth import external_account_authorized_user
...
def _get_impersonated_service_account_credentials(filename, info, scopes):
    from lib.google.auth import impersonated_credentials
...
def _get_gdch_service_account_credentials(filename, info):
    from lib.google.oauth2 import gdch_credentials
...
def get_api_key_credentials(key):
    ...
    from lib.google.auth import api_key
...
def _apply_quota_project_id(credentials, quota_project_id):
    ...
    from lib.google.oauth2 import credentials as authorized_user_credentials
...
def default(scopes=None, request=None, quota_project_id=None, default_scopes=None):
    ...
    Raises:
        ~google.auth.exceptions.DefaultCredentialsError:
            ...
            invalid.
    """
    from lib.google.auth.credentials import with_scopes_if_required
    from lib.google.auth.credentials import CredentialsWithQuotaProject
